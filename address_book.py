...
class Phone(Field):
    def __init__(self, value):
        if not value.isdigit() or len(value) != 10:
            raise ValueError("Phone number not valid. It should be a 10-digit number.")
        super().__init__(value)
...
class Record:
    ...
    def remove_phone(self, phone_str):
        ...
        # self.phones = list(filter(lambda phone: phone.value != phone_str, self.phones))
...
class AddressBook(UserDict):
    ...
    def __str__(self):
        return F"Address Book: {', '.join(str(record) for record in self.data.values())}"
